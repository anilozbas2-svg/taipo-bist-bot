...
def build_hourly_message(state):
    ...
    # 6 hisseyi iki blok gibi göstermek (ilk 3 güçlü, ikinci 3 zayıf)
    # Sıralama: state'e kaydederken güçlüler + zayıflar şeklinde kayıt edeceğiz.
...
def build_no_watch_message():
    # Eğer daha seçim yoksa /taipo komutuna premium cevap:
    lines = []
...
def run_auto():
    """
    main.yml ile tetiklenir.
    - 09:00-09:02 arası ilk yakalarsa: 6 hisse seç + açılış mesajı at
    - gün içinde saatlik takip: aynı 6 hisseyi göster
    - 20:55 gün sonu raporu
    """
    ensure_files()
    ...
    if not symbols:
        # sembol listesi yoksa uyarı bas
...
def run_command_listener():
    ...
    for upd in updates:
        ...
        if text.lower().startswith("/taipo"):
            ...
            # Eğer bugün henüz seçim yoksa: anlık tara ve göster (3 güçlü + 3 zayıf)
            if not watch_syms:
                symbols = load_symbols()
                if symbols:
                    quotes = scan_quotes(symbols)
                ...
                # seçim varsa: premium takip mesajı döndür (komuta özel chat_id)
